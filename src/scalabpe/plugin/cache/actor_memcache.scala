...
import MemCacheSoc.TYPE_UNKNOWN
...
class MemCacheActor(val router: Router, val cfgNode: Node)
    ...
    def init() {
        ...
        threadFactory = new NamedThreadFactory("memcacheactor-" + firstServiceId)
        ...
        soc = new MemCacheSoc(addrs, cacheType, this.receive, connectTimeout, pingInterval,
            connSizePerAddr, timerInterval, reconnectInterval,
            failOver, maxErrorCount)

        log.info("MemCacheActor started {}", serviceIds)
    }
    ...
    def close() {
        ...
        if (t2 - t1 > 100)
            log.warn("MemCacheActor long time to shutdown pool, ts={}", t2 - t1)

        soc.close()
        log.info("MemCacheActor stopped {}", serviceIds)
    }
    ...
    override def receive(v: Any): Unit = {
        ...
        try {
            pool.execute(new Runnable() {
                def run() {
                    try {
                        ...
                        case e: Exception =>
                            log.error("MemCacheActor exception v={}", v, e)
                    }
                }
            })
        } catch {
            case e: RejectedExecutionException =>
                ...
                log.error("MemCacheActor queue is full, serviceIds={}", serviceIds)
        }
    }
