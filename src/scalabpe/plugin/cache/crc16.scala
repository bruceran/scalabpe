...
object Crc16 {
    ...
    val LOOKUP_TABLE = Array[Int](0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5,
    ...
    def getCRC16(bytes: Array[Byte], s: Int, e: Int): Int = {
        var crc = 0x0000;

        var i = s
        while (i < e) {
            val t = ((crc >>> 8) ^ (bytes(i) & 0xFF)) & 0xFF
            crc = ((crc << 8) ^ LOOKUP_TABLE(t));
            i += 1
        }
        return crc & 0xFFFF;
    }

    def getCRC16(key: String): Int = {
        val bytesKey = key.getBytes("utf-8")
        getCRC16(bytesKey, 0, bytesKey.length);
    }
