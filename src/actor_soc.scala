...
class SocActor(val router: Router,val cfgNode: Node) extends Actor with RawRequestActor with Logging with Closable with SelfCheckLike  with Dumpable  {
    ...
    def init() {
      ...
      socWrapper = new SocImpl(addrs,router.codecs,this.receive,
      	retryTimes,connectTimeout,pingInterval,maxPackageSize,connSizePerAddr,timerInterval,reconnectInterval,
        isSps,reportSpsTo,
      	actor = this)
...
class SocCacheData(val data:Any, val sendTime:Long, val timeout:Int, var sendTimes:Int) {
...
class SocImpl(
  ...
  val dataMap = new ConcurrentHashMap[Int,SocCacheData]()
  ...
  def send(rawReq: RawRequest,
      ...
      val data = new AvenueData(
            ...
            req.code,
            req.xhead, req.body )
      dataMap.put(sequence,new SocCacheData(rawReq,timeout))
  ...
  def send(req: Request, timeout: Int):Unit = {
        ...
        val data = new AvenueData(
              ...
              0,
              xhead, body )
        dataMap.put(sequence,new SocCacheData(req,timeout))
