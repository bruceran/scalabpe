...
class NettyClientHandler(val nettyClient: NettyClient) extends IdleStateAwareChannelHandler with Logging  {
...
class NettyTimeoutInfo(val sequence:Int, val connId:String, val timer: QuickTimer)
...
class NettyClient(
  ...
  var nettyClientHandler : NettyClientHandler = _

  val addrs = addrstr.split(",")
  val dataMap = new ConcurrentHashMap[Int,NettyTimeoutInfo]()
  ...
  def nextChannelFromMap(sequence:Int,timeout:Int,connId:String) :Channel = {
    ...
    try {
        ...
        if( ch.isOpen ) {
          ...
          val ti = new NettyTimeoutInfo(sequence,connId,t)
